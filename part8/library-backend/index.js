 ...
 *
 * English:
 * It might make more sense to associate a book
 * with its author by storing the author's id in the context
...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) => {
      ...
      if (args.genres) {
        return books.filter((book) => book.genres === args.genres);
      }
      return books;
    },
    allAuthors: () => authors,
  ...
  Mutation: {
    addBook: (root, args) => {
        const book = { ...args.book, id: uuidv4() };
        books.push(book);
        return book;
    }
